# Script Title: Individual Transect Interpolated Salinity Gradient Visualizer
...
# Interpolates and visualizes the salinity gradient versus depth for each transect,
...
SAVE_DIR = os.path.join(BASE_DIR, 'visualization/contour_plots/cf/salinity/contour_interpolation')
...
def plot_salinity_gradients(file_names):
    for idx, fname in enumerate(file_names):
        ...
        contour = ax.contourf(xi, yi, zi, 100, cmap=cmocean.cm.haline, vmin=local_min, vmax=local_max)  # Modified colormap
        cbar = plt.colorbar(contour)
        cbar.set_label('Salinity (PSU)')  # Adjusted unit
        ...
        file_save_path = f"{SAVE_DIR}/salinity_gradient_transect_{idx + 1}.png"
        try:
