...
SAVE_DIR = os.path.join(BASE_DIR, 'visualization/contour_plots/wb/chlorophyll/3D_plots/accumulating_transects')
...
def get_global_min_max(file_names):
    ...
    print("Processing global minimum and maximum chlorophyll values...")
    for fname in tqdm(file_names):
        try:
            data = pd.read_excel(fname)
            all_mins.append(data['chlor_a'].min())
            all_maxs.append(data['chlor_a'].max())
...
def plot_cumulative_transects_3d_chlorophyll_gradient(file_names, global_min, global_max, upto_idx):
    fig = plt.figure(figsize=(12, 10))
    ax = fig.add_subplot(111, projection='3d')
    
    distances_from_shore = []
    
    # Processing file names and creating dataframes
    ...
    for idx, (distance, df, fname) in enumerate(sorted_files_and_dfs[:upto_idx + 1]):
        ...
        # Calculate an alpha value with more pronounced steps
        # The larger the multiplier, the more pronounced the steps in transparency
        multiplier = 0.9  # Adjust this value to control the step size
        alpha_value = max(0, 1 - (multiplier * idx / (len(sorted_files_and_dfs) - 1)))

        # Plotting
        ax.scatter(xi, yi, zi, c=colors.flatten(), cmap=cmocean.cm.algae, marker='o', alpha=1, vmin=global_min, vmax=global_max)

        # Define the levels where we want to highlight chlorophyll values
        chlorophyll_contours = np.arange(1.0, global_max + 1, 1.0)
        
        # Create filled contours to outline regions where chlorophyll values exceed the threshold
        CS = ax.contourf(xi, yi, zi, levels=chlorophyll_contours, cmap='Reds', alpha=0.5)
    ...
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=ax, ticks=np.linspace(global_min, global_max, 11)) 
...
def main():
    ...
    print(f"Global Minimum Chlorophyll: {global_min} µg/L")
    print(f"Global Maximum Chlorophyll: {global_max} µg/L")

    for idx in range(len(file_names)):
        plot_cumulative_transects_3d_chlorophyll_gradient(file_names, global_min, global_max, idx)
    
    print("3D visualizations complete!")

    # New code to generate GIF
    gif_save_path = os.path.join(SAVE_DIR, 'chlor_animation.gif')
