# Script Title: Turbidity Depth Profiles Visualization (Skipping 6th transect)
# Author: Mitch Torkelson
# Date: August 2023

# Description:
# This script visualizes the depth profiles for turbidity data from "cleaned"
# transect data files (excluding the 6th transect). For each transect, two plots are produced side by side:
...
SAVE_DIR = os.path.join(BASE_DIR, 'visualization/depth_profiles/turbidity_skip6')
FILE_NAMES = [os.path.join(DATA_DIR, f'cleaned_data_transect_{i}.xlsx') for i in range(1, 8) if i != 6]
...
def main():
    ...
    for i, df in zip([i for i in range(1, 8) if i != 6], all_data):
