...
#-------------------------------------------------------------------------------
    ...
    if west > 0.0 and east > 180.0:
        ...
        center_lon = west + abs(west - east)/2.0
...
#-------------------------------------------------------------------------------
    ...
    if ydim_latlon != ydim_data or xdim_latlon != xdim_data:
        ...
        im=Image.fromarray(l2_lon)
        im= im.resize(l2_data.shape, resample=Image.BILINEAR)
        l2_lon= np.array(im)

        im=Image.fromarray(l2_lat)
        im= im.resize(l2_data.shape, resample=Image.BILINEAR)
        l2_lat= np.array(im)
    ...
    slon_ctr= str(lon_0).strip()

    #p = pyproj.Proj('+proj=eqc +lat_ts=0 +lat_0=0 +lon_0=0 +x_0=0 +y_0=0 +a=6378137 +b=6378137 +units=m')
    p = pyproj.Proj('+proj=eqc +lat_ts=0 +lat_0=0 +lon_0=' + slon_ctr + ' +x_0=0 +y_0=0 +a=6378137 +b=6378137 +units=m')
    ...
    proj4_args = '+proj=eqc +lat_ts=0 +lat_0=0 +lon_0=' + slon_ctr + ' +x_0=0 +y_0=0 +a=6378137 +b=6378137 +units=m'
    ...
    area_def = AreaDefinition(area_id, area_name, proj_id, proj4_args, xdim, ydim, area_extent)
...
#----------------------------------------------------------------------------
    ...
    if global_smi_chk == True: center_lon = 0.0

    if proj_name == 'Equidistant Cylindrical' or proj_name == 'RECT': proj_name = 'Cylindrical'



    north= float(latlon.north)
    west=  float(latlon.west)
    south= float(latlon.south)
    east=  float(latlon.east)

    lat_center, lon_center =  compute_cntr_latlon(south,west,north,east)

    print('\nWest= ', west, '\nEast= ',east)
    print('Center Longitude= ', lon_center)
    #-------------------------------------------------------------------#
    ...
    seadas_lut_resources = ocsswroot_path + '/share/common/palette'
    ...
    geophys_img = np.where(geophys_img == -32767, np.nan, geophys_img)
...
#-----------------------------------------------------------------------
    ...
    if global_smi_chk is True:  mapres='50m'
    if global_smi_chk is False: mapres='10m'

    print('\nCreating PNG file with aded coastline using the following resolution... ', mapres, '\n')

    #geophys_img= flipud(geophys_img) # <----<<< flip array upside down only for mapping... commented out Jan 4 2022
...
#-----------------------------------------------------------------------
    ...
    if west > 180 or east > 180: lon_ctr=180.0
    else: lon_ctr=0.0

    print('\n\nusing lon_ctr...  ', lon_ctr, '\n\n')

    map_data_extent=map_data_extent=[west, east, south, north]
    ...
    if proj_name == 'mollweide':
          ax= plt.axes(projection=ccrs.Mollweide(central_longitude=lon_ctr))
          ax.set_extent(map_data_extent, crs=ccrs.Mollweide())

    if proj_name == 'lambert':
        ax= plt.axes(projection=ccrs.LambertConformal(central_longitude=lon_ctr))
        ax.set_extent(map_data_extent, crs=ccrs.LambertConformal())

    if proj_name == 'albersconic':
        ax= plt.axes(projection=ccrs.AlbersEqualArea(central_longitude=lon_ctr))
        ax.set_extent(map_data_extent, crs=ccrs.AlbersEqualArea())


    #commented this out Jan 4 2022
    ...
    #if scale_type == 'LOG': sat_img= ax.imshow(geophys_img,  transform=ccrs.PlateCarree(), extent=map_data_extent, cmap=mycmap, vmin=low_limit, vmax=upper_limit,norm=matplotlib.colors.LogNorm())

    #Replaced Lines Above with the following on Jan 4 2022
    if scale_type == 'LIN': sat_img= ax.imshow(geophys_img,  transform=ccrs.PlateCarree(), extent=map_data_extent, cmap=cmocean.cm.deep, vmin=low_limit, vmax=upper_limit)
    if scale_type == 'LOG': sat_img= ax.imshow(geophys_img,  transform=ccrs.PlateCarree(), extent=map_data_extent, cmap=cmocean.cm.deep, norm=matplotlib.colors.LogNorm(low_limit,upper_limit))
...
#-----------------------------------------------------------------------
    ...
    # ---
    feature = cartopy.feature.GSHHSFeature(scale='full', levels=[1], facecolor='gray', edgecolor='black')
    ax.add_feature(feature)


    # Setup and draw Lat/Lon Labels..
    delta_lon=abs(west - east)
    delta_lat=abs(north-south)

    if delta_lon < 4:                      lon_space=0.05
    if delta_lon >=  4 and delta_lon < 8:  lon_space=1
    if delta_lon >=  8 and delta_lon < 16: lon_space=2
    if delta_lon >= 16:                    lon_space=5

    if delta_lat < 4:                      lat_space=0.05
    if delta_lat >=  4 and delta_lat < 8:  lat_space=1
    if delta_lat >=  8 and delta_lat < 16: lat_space=2
    if delta_lat >= 16:                    lat_space=5


    meridians = np.arange(west, east,lon_space)
    parallels = np.arange(south, north,lat_space)
    # ---
    ...
    if proj_name == 'mollweide':
        ax.set_xticks(meridians, crs=ccrs.Mollweide())
        ax.set_yticks(parallels, crs=ccrs.Mollweide())

    if proj_name == 'lambert':
        ax.set_xticks(meridians, crs=ccrs.lambert())
        ax.set_yticks(parallels, crs=ccrs.lambert())

    if proj_name == 'albersconic':
        ax.set_xticks(meridians, crs=ccrs.albersconic())
        ax.set_yticks(parallels, crs=ccrs.albersconic())
...
#----------------------------------------------------------------------------------------------------
    ...
    if sst_mask_chk == True:
       ...
       mask_img= ones(qual_img.shape)                # make a 2D array of ones that has the same dimensions as qual_img
    ...
    if sst_mask_chk == False:
        ...
        flags_to_check=sort(flags_to_check)

        print('\n\nflags to check >>>>----->  ', flags_to_check, '\n\n')
...
# Expect input string of the form: fname= 'A20004hhmmss...'
    ...
    new_format_l2fname= fname_new_from_old(rootname)
    mission_instrument= new_format_l2fname.split('.')[0]
    instrument=  mission_instrument.split('_')[1]


    if instrument == 'SEAWIFS': sat_resoulution = 1000.0

    if instrument == 'MODIS' and 'HKM' not in new_format_l2fname and 'QKM' not in new_format_l2fname: sat_resoulution = 1000.0
    if instrument == 'MODIS' and 'HKM' in new_format_l2fname: sat_resoulution = 500.0
    if instrument == 'MODIS' and 'QKM' in new_format_l2fname: sat_resoulution = 250.0

    if instrument == 'MSI': sat_resoulution = 30.0

    if instrument == 'MERIS' and 'FRS' not in new_format_l2fname: sat_resoulution = 1000.0
    if instrument == 'MERIS' and 'FRS' in new_format_l2fname: sat_resoulution = 300.0


    if instrument == 'OLCI' and 'EFR' in new_format_l2fname: sat_resoulution = 300.0
    if instrument == 'OLCI' and 'ERR' in new_format_l2fname: sat_resoulution = 1000.0

    if instrument == 'VIIRS': sat_resoulution = 750.0

    if instrument == 'HICO': sat_resoulution = 90.0

    if instrument == 'OLI': sat_resoulution = 30.0

    if instrument == 'HAWKEYE': sat_resoulution = 75.0    #75.0

    if instrument == 'OCI': sat_resoulution = 1000.0

    return sat_resoulution


def convert_l2binres_2_l3mapres(l2bin_resolution):

    if l2bin_resolution == 'H': l3mapgen_resolution= '0.5km'
    if l2bin_resolution == 'Q': l3mapgen_resolution= '250'
    if l2bin_resolution == 'HQ': l3mapgen_resolution= '100'
    if l2bin_resolution == 'HH': l3mapgen_resolution= '50'
    if l2bin_resolution == '1': l3mapgen_resolution= '1.1km'
    if l2bin_resolution == '2': l3mapgen_resolution= '2.3km'
    if l2bin_resolution == '4': l3mapgen_resolution= '4.6km'
    if l2bin_resolution == '9': l3mapgen_resolution= '9.2km'
    if l2bin_resolution == '18': l3mapgen_resolution= '18.5km'

    return l3mapgen_resolution
...
#-------------------------------------------------------------------------
    ...
    if len(requested_prod) < 20: print('requested product ---> ', requested_prod)
    if len(all_prod) < 20:       print('all products --------> ', all_prod)
...
#-------------------------------------------------------------------------
    ...
    print('\nGetting L2 Products... ', sensor_id,prod_type, '\n')

    #  input:
    #  sensor_id ==> INSTRUMENT NAME (e.g., SEAWIFS, OLCI...)'
    #  prod_type ==> products ('color' or 'sst')
    #
    # output:
    # comma-separated string of flags ('ATMFAIL,LAND,HILT,HISATZEN,STRAYLIGHT,CLDICE,...') <------<<<

    if sensor_id == 'SEAWIFS' and prod_type == 'color':
        fname = '$OCSSWROOT/share/seawifs/l2bin_defaults.par'
    if sensor_id == 'MODIS' and prod_type == 'color':
        fname = '$OCSSWROOT/share/modis/l2bin_defaults.par'
    if sensor_id == 'MERIS' and prod_type == 'color':
        fname = '$OCSSWROOT/share/meris/l2bin_defaults.par'
    if sensor_id == 'VIIRS' and prod_type == 'color':
        fname = '$OCSSWROOT/share/viirs/l2bin_defaults.par'
    if sensor_id == 'CZCS' and prod_type == 'color':
        fname = '$OCSSWROOT/share/czcs/l2bin_defaults.par'
    if sensor_id == 'HICO' and prod_type == 'color':
        fname = '$OCSSWROOT/share/hico/l2bin_defaults.par'
    if sensor_id == 'OLCI' and prod_type == 'color':
        fname = '$OCSSWROOT/share/olci/l2bin_defaults.par'
    if sensor_id == 'OLI' and prod_type == 'color':
       #fname = '$OCSSWROOT/share/oli/l2bin_defaults.par'
        fname = '$OCSSWROOT/share/modis/l2bin_defaults.par'

    if sensor_id == 'HAWKEYE' and prod_type == 'color':
        #fname = '$OCSSWROOT/share/hawkeye/l2bin_defaults.par'
        fname = '$OCSSWROOT/share/modis/l2bin_defaults.par'
    if sensor_id == 'OCI' and prod_type == 'color':
        #fname = '$OCSSWROOT/share/oci/l2bin_defaults.par'
        fname = '$OCSSWROOT/share/modis/l2bin_defaults.par'

        print('\nNOTE!!!  Processing oli or hawkeye or oci file will use modis l2bin_defaults.par file because')
        print('of missing oli defaults file: $OCSSWROOT/share/mis/l2bin_defaults.par as of')
        print('June 1, 2018.   This should be corrected when missing l2bin_defaults becomes')
        print('available.  The needed change is in  get_sds7_default_l2flags function')
        print('found in my_mapping_utilities... The only thing being pulled from this file')
        print('the default l2flags that are used in the binning process.\n')


    if sensor_id == 'MODIS' and prod_type == 'sst':
        fname = '$OCSSWROOT/share/modis/l2bin_defaults_SST.par'
    if sensor_id == 'VIIRS' and prod_type == 'sst':
        fname = '$OCSSWROOT/share/modis/l2bin_defaults_SST.par'

    if sensor_id == 'MSI' and prod_type == 'color':
        fname = '$OCSSWROOT/share/modis/l2bin_defaults.par'
        print('\nNOTE!!!  Processing mis file using modis l2bin_defaults.par file because')
        print('of missing msi defaults file: $OCSSWROOT/share/mis/l2bin_defaults.par as of')
        print('June 1, 2018.   This should be corrected when mis l2bin_defaults becomes')
        print('available.  The needed change is in  get_sds7_default_l2flags function')
        print('found in my_mapping_utilities... The only thing being pulled from this file')
        print('the default l2flags that are used in the binning process.\n')
    ...
    flags = result.split('=')
    if prod_type == 'sst': flags[1] = flags[1] + ',CLDICE,NAVWARN,SEAICE,SSTWARN,SSTFAIL'
...
def get_prod_min_max(fname, prod):
    ...
    # all lines reads in as a one-element list containing a single long string
    # of prod-min-max-scale spearated by \r charage retrun symbol.  The following
    # breaks the long string into a list with each element in the list a single
    # prod-min-max-scale... This is needed for python v2.7, but not for 3.5. Don't
    # ask me why///
    # ---
    if sys.version_info[0] == 2: all_lines=all_lines[0].split('\r')

    prod_min_max_scale= ['']
    for line in all_lines:
        if prod in line and len(line.split()[0]) == len(prod):
            prod_min_max_scale= line.split()
    if len(prod_min_max_scale) == 0:
        print('#'*50)
        ...
        prod_vec =  prod_min_max_scale[0]
        minval_vec = prod_min_max_scale[1]
        maxval_vec = prod_min_max_scale[2]
        scaletype_vec =prod_min_max_scale[3]
...
def png_rescale(indir, product):
    ...
    for ifile in fname_list:
        ...
        if ifile.find('.smi.') != -1:
            mappping_approach= 'binmap' #string .smi. found (it did not retrun a -1 when it made the string search)

        if ifile.find('.map.') != -1:
            mappping_approach= 'str_map' #string .map. found (it did not retrun a -1 when it made the string search)
