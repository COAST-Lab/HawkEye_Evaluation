# Script Title: Individual Transect Interpolated Turbidity Gradient Visualizer
...
# Interpolates and visualizes the turbidity gradient versus depth for each transect,
...
SAVE_DIR = os.path.join(BASE_DIR, 'visualization/contour_plots/cf/turbidity/contour_interpolation')
...
def plot_turbidity_gradients(file_names):
    for idx, fname in enumerate(file_names):
        ...
        zi = griddata((df['normalized_distance'], df['depth']), df['turbidity'], (xi, yi), method='linear')
        zi = np.ma.masked_invalid(zi)

        local_min = df['turbidity'].min()
        local_max = df['turbidity'].max()
        print(f"Local Turbidity Min for transect {idx + 1}: {local_min}")
        print(f"Local Turbidity Max for transect {idx + 1}: {local_max}")

        fig, ax = plt.subplots(figsize=(10, 6))

        contour = ax.contourf(xi, yi, zi, 100, cmap=cmocean.cm.turbid, vmin=local_min, vmax=local_max) # Modified colormap
        cbar = plt.colorbar(contour)
        cbar.set_label('Turbidity (NTU)') # Adjusted unit
        ...
        ax.set_title(f'Turbidity gradient of transect {idx + 1}')

        file_save_path = f"{SAVE_DIR}/turbidity_gradient_transect_{idx + 1}.png"
        try:
