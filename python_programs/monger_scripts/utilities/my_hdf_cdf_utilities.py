...
def read_hdf_prod(ifile,prod,nc_autoscale=False):
#---------------------------------------------------------------
    ...
    if ftype == 'hdf5':
        ...
        if nc_autoscale==False:
            ...
            print("TO TURN ON AUTOSCALE USE THE FOLLWING: data=read_hdf_prod(fname, prodname, nc_autoscale='True')")
            print('----------------------------------------------------------------------------------------------------\n')
    ...
    if ftype == 'hdf5':
        ...
        if len(group_names) != 0:
            ...
            if group_names[0] != 'processing_control':
                ...
                for grp_name in group_names:
                    print('\nGroup Name= ',  grp_name)
                    var_name= list(f.groups[grp_name].variables.keys())
    ...
    if ftype == 'hdf5':
        ...
        if len(group_names) != 0:
            ...
            # necdf4 file has groups, but the only group is 'processing_control' and that is a smi file and the variable are at the top level
               ...
               var_name= list(f.variables.keys())  #get names of objects within each group (e,g. chlor_a or longitude)
               for vn in var_name:
                   if vn == prod:
                       p =  f.variables[vn]
                       print(p)
...
#---------------------------------------------------------------
    ...
    if ftype == 'hdf5':
        ...
        var_name= list(f.groups['geophysical_data'].variables.keys())
        var_name= sorted(var_name)
...
#---------------------------------------------------------------
    ...
    #projection= unicode(projection)
    f.close()

    if 'eqc' in projection: projection='platecarree'
    if 'moll' in projection: projection='mollweide'
    if 'lcc' in projection: projection='lambert'
    if 'aea' in projection: projection='albersconic'
...
def get_l2hdf_full_l2flags_names(ifile):
    ...
    if ftype == 'hdf4':
        ...
        flag_names_vec= np.asarray(['ATMFAIL','LAND' ,'PRODWARN' ,'HIGLINT' ,'HILT' ,'HISATZEN' ,'COASTZ' ,'SPARE' ,'STRAYLIGHT' ,'CLDICE' ,'COCCOLITH' ,'TURBIDW' ,'HISOLZEN' ,'SPARE' , 'LOWLW' ,'CHLFAIL' ,'NAVWARN' ,'ABSAER' ,'SPARE' ,'MAXAERITER' ,'MODGLINT' ,'CHLWARN' ,'ATMWARN' ,'SPARE' ,'SEAICE' ,'NAVFAIL' ,'FILTER' ,  'SPARE' ,'BOWTIEDEL' ,'HIPOL' ,'PRODFAIL' ,'SPARE'], dtype='|S8')
